...
export function provideBuilder() {
  return class CargoBuildProvider {
    ...
    settings() {
      ...
      // The last build workding directory
      // (might differ from the project root for multi-crate projects)
      let buildWorkDir;
        ...
        output.split(/\n/).forEach(line => {
          ...
          if (line === null || line === '' || line.substring(0, 6) === 'error:') {
            ...
            if (match) {
              ...
              if (match) {
                const linkId = new Date().getTime() + '::' + match[2] + '::' + match[3];
                atom.notifications.addError(
                  'A thread panicked at' + ' <a id="' + linkId + '" href="#">line ' + match[3] + ' in ' + match[2] + '</a>', {
                    detail: match[1],
                    stack: 'Stack trace',
                    dismissable: true
                });
                const link = document.getElementById(linkId);
                link.buildWorkDir = buildWorkDir; // Preserve it for using even if another build was executed later
                if (link) {
                  link.addEventListener('click', function(e) {
                    const l = e.target;
                    const idParts = l.id.split('::');
                    atom.workspace.open(path.join(l.buildWorkDir, idParts[1]), {
                      searchAllPanes: true,
                      initialLine: parseInt(idParts[2]) - 1
                    });
                  });
                }
/*                msg = {
