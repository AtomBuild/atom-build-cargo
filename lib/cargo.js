...
export function provideBuilder() {
  return class CargoBuildProvider {
    ...
    settings() {
      ...
      let panicCounter = 0; // Counts panics
      ...
      function showPanic(panic) {
        atom.notifications.addError(
          'A thread panicked at' + ' <a id="' + panic.id + '" href="#">line ' + panic.line + ' in ' + panic.file + '</a>', {
      ...
      // parsed lines.
      function tryParsePanic(lines, i) {
        const line = lines[i];
        ...
        if (match) {
          parsedQty = 1;
          const panic = {
            id: 'build-cargo-panic-' + (++panicCounter), // Unique panic ID
            message: match[1],
            file: match[2],
            filePath: path.join(buildWorkDir, match[2]),
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (line === null || line === '' || line.substring(0, 6) === 'error:') {
            ...
            if (match) {
              ...
              // Check for panic
              const parsedQty = tryParsePanic(lines, i);
              if (parsedQty > 0) {
                msg = null;
                sub = null;
